...
for i in range(numJobs):
    ...
    cpuPercent = (float(job[i]['runTime']) / turnaround) * 100 if turnaround > 0 else 0
    print('  Job %2d: startTime %3d - response %3d - turnaround %3d - CPU %.2f%%' % (i, job[i]['startTime'], response, turnaround, cpuPercent))
    responseSum   += response
    turnaroundSum += turnaround
    cpuSum        += cpuPercent

print('\n  Avg %2d: startTime n/a - response %.2f - turnaround %.2f - CPU %.2f%%' % (i, float(responseSum)/numJobs, float(turnaroundSum)/numJobs, float(cpuSum)/numJobs))
print('\n')
